...
_EN = {
    ...
    "空 Tag 处理": "Empty Tag handling",
    "并发下载数": "Concurrent downloads",
    "单文件分片数": "Chunks per file",
    "Telegram 单文件分片数": "Telegram chunks per file",
    "大文件可尝试 2-4；不稳定或限流时保持 1。": "Try 2-4 for large files; keep 1 if unstable or rate-limited.",
    ...
    "将已下载图片与 IGP sidecar 融合为元数据图片或 .igp 包。": "Fuse downloaded images and IGP sidecars into metadata images or .igp packages.",
    "来源": "Source",
    "选择图片、.igp.json 或包含它们的目录": "Choose an image, .igp.json, or a folder containing them",
    "选择文件": "Choose file",
    "选择目录": "Choose folder",
    "递归扫描子目录": "Scan subfolders recursively",
    "来源为目录时，递归查找所有匹配的图片 + .igp.json。": "When the source is a folder, recursively find all matched image + .igp.json pairs.",
    "输出目录": "Output folder",
    "留空则输出到原文件所在目录": "Leave empty to output next to each original file",
    "导出模式": "Export mode",
    "IGP 格式": "IGP format",
    "嵌入图片元数据": "Embed image metadata",
    "生成单图 .igp 包，可选择写入哪些扩展信息。": "Create single-image .igp packages and choose which metadata sections to include.",
    "生成带 IGP metadata 的 JPEG/PNG 副本。": "Create JPEG/PNG copies with embedded IGP metadata.",
    "开始导出": "Start export",
    "说明": "Notes",
    "只会处理严格匹配的文件对：": "Only exactly matched file pairs are processed:",
    "孤立图片或孤立 sidecar 会跳过，不会生成输出。": "Orphan images or sidecars are skipped and do not generate output.",
    "当前状态": "Current status",
    "等待导出任务。": "Waiting for an export task.",
    "打开输出目录": "Open output folder",
    "导出结果": "Export results",
    "文件": "File",
    "模式": "Mode",
    "输出": "Output",
    "请先选择来源。": "Choose a source first.",
    "请先选择来源": "Choose a source first",
    "来源不存在": "Source does not exist",
    "导出模式无效": "Invalid export mode",
    "没有找到可导出的匹配文件": "No matched files found for export",
    "导出完成": "Export complete",
    "导出完成，部分文件失败": "Export complete with some failures",
    "IGP 打包选项": "IGP package options",
    "选择要写入 IGP 的信息": "Choose what to include in the IGP",
    "原图始终会进入 .igp；下面只控制附加 metadata 段。": "The original image is always included; the options below control extra metadata sections.",
    "图片基础信息": "Image basics",
    "文件名、MIME、尺寸、大小与 SHA-256。": "Filename, MIME, dimensions, size, and SHA-256.",
    "Tags": "Tags",
    "任务 Tag、Telegram 话题标签和来源标签。": "Task Tags, Telegram hashtags, and source tags.",
    "Telegram 来源": "Telegram source",
    "频道、帖子、消息、媒体 ID 与来源链接。": "Channel, post, message, media ID, and source links.",
    "帖子文本": "Post text",
    "匹配帖子、评论或资源消息中的文本。": "Text from the matched post, comment, or resource message.",
    "下载记录": "Download record",
    "本地文件名与保存目录等导入记录。": "Import record such as local filename and save folder.",
    "其它扩展字段": "Other extension fields",
    "保留未来新增或第三方写入的未知 metadata 段。": "Keep unknown metadata sections added in the future or by third parties.",
    "校验清单": "Checksum list",
    "额外写入 checksums.json，用于之后校验包内文件完整性。": "Write checksums.json so package contents can be verified later.",
    "最小包": "Minimal package",
    "全选": "Select all",
    "选择图片或 IGP sidecar": "Choose image or IGP sidecar",
    "选择来源目录": "Choose source folder",
...
_PREFIX_EN = {
    ...
    "预览：": "Preview: ",
    "已启用：": "Enabled: ",
    "没有找到可导出的匹配文件。": "No matched files found. ",
    "导出完成：": "Export complete: ",
}
