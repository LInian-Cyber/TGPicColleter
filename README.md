...
- 支持识别 Telegraph / graph.org 漫画页，下载页面图片并自动合成为 PDF。
...
已忽略的本地文件包括 `.venv/`、`build/`、`dist/`、`tg-api.txt`、`.session`、`*.sqlite3`、`*.db`、`ui/` 设计稿和本地临时目录。
...
- 新增 Telegraph / graph.org 漫画资源模式，可在任务页单独下载漫画或与帖子图片同时下载。
- 搜索预览支持显示 Telegraph 漫画页命中来源、漫画页数量，并在结果中勾选要直接下载的帖子。
- Telegraph 漫画下载会自动拉取封面与正文图片并合成为 PDF，修复中文页面地址编码失败和封面遗漏问题。
- Telegraph 漫画默认只保留 PDF，可在设置页开启保留单页原图；PDF 保存路径改为 `漫画/页面名/页面名.pdf`。
- 修复私有频道下拉框只显示频道 ID 的问题，频道历史会显示频道名、头像缓存和 `t.me/c/...` 地址。
- 将运行态配置与媒体索引迁移到 SQLite，本地数据库文件已加入忽略规则，避免误提交。
- 拆分 `controller.py`、`telegram_worker.py` 中的 Yande、导出、消息解析、媒体索引和 Telegraph 逻辑，降低大文件维护成本。
- 补充下载分片、媒体去重、恢复任务、配置保存、Yande 解析和 Windows 保留名相关测试。
- 补充 Telegraph 链接解析、PDF 合成、中文 URL 编码、预览勾选下载、私有频道显示和 SQLite 索引相关测试。
